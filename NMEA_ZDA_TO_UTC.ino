...
// Remarques  : Ajout d'une limite maximale pour DeltaUTC (Protection anti rollover de millis() et autres délais possible dans la communication GPS/Artemis).
// 
    ...
    if ( debutNMEA &&  finNMEA ) {  // Le message NMEA courant est complet.
      ...
      if (receivedChars[0] == '$' && receivedChars[1] == 'G' && receivedChars[2] == 'N' && receivedChars[3] == 'R' && receivedChars[4] == 'M' && receivedChars[5] == 'C' && receivedChars[6] == ',' ) { // Message "$GNRMC," détecté. Extraire l'heure UTC.
        ...
        if (DeltaUTC >= 740UL){                             // Si délais anormalement long de 740 mS. Rollover de millis() après 49 jours ? Autres ?
          Serial.write( "2591\r\n" );                       // Impose correction arbitraire à 259.1 mS. Le "".2591" est facile à cherche dans les logs car normalement le 4e chiffre après le ppint est toujours zéro.
        else {  
      ...
      // Message $GNZDA
        ...
        // Ce n'est pas grave si le message "$GNZDA," est entrecoupé d'un délais d'environ 750 mS car stratégiquement on ne garde que sa date. 
        ...
        // Autre BUG potentiel si tu reste dans to Kayak plus de 49 jours tu risque d'avoir un bug dans l'heure UTC car le compteur millis vas faire un rollover de 4294967295 à zéro. Désolé ! :)
